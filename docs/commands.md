...
```bash
> polygonetl export_blocks_and_transactions -h

Usage: polygonetl export_blocks_and_transactions [OPTIONS]
...
```bash
> polygonetl extract_csv_column --input transactions.csv --column hash --output transaction_hashes.txt
```

Then export receipts and logs:

```bash
> polygonetl export_receipts_and_logs --transaction-hashes transaction_hashes.txt \
...
```bash
> polygonetl extract_token_transfers --logs logs.csv --output token_transfers.csv
```
...
```bash
> polygonetl extract_csv_column --input receipts.csv --column contract_address --output contract_addresses.txt
```

Then export contracts:

```bash
> polygonetl export_contracts --contract-addresses contract_addresses.txt \
...
```bash
> polygonetl filter_items -i contracts.json -p "item['is_erc20'] or item['is_erc721']" | \
polygonetl extract_field -f address -o token_addresses.txt
```

Then export ERC20 / ERC721 tokens:

```bash
> polygonetl export_tokens --token-addresses token_addresses.txt \
...
```bash
> polygonetl export_geth_traces --start-block 0 --end-block 500000 \
...
```bash
> polygonetl extract_geth_traces --input geth_traces.json --output traces.csv
```
...
```bash
> polygonetl get_block_range_for_date --provider-uri=https://mainnet.infura.io/v3/7aef3f0cd1f64408b163814b22cc643c --date 2018-01-01
4832686,4838611
```
...
```bash
> polygonetl get_keccak_hash -i "transfer(address,uint256)"
...
> polygonetl stream --provider-uri https://mainnet.infura.io/v3/7aef3f0cd1f64408b163814b22cc643c --start-block 500000
```
...
> polygonetl stream --start-block 500000 --output projects/<your-project>/topics/crypto_ethereum
```
